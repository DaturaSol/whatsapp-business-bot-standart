...
AnyTemplateComponent = Union[
    StaticHeaderComponent, StaticBodyComponent, FlowButtonComponent
]
        ...
        **kwargs,
        ...
        if has_flow_button:
            if flow_button_index is None:
                raise ValueError(
                    "flow_button_index is required if has_flow_button is True"
                )

            action_payload_obj = FlowActionPayload(
                flow_token=flow_token, flow_action_data=flow_action_data_payload
            )
            ...
            flow_button_component = FlowButtonComponent(
                index=flow_button_index, parameters=[action_parameter_obj]
            )
            components_payload.append(flow_button_component)

        template_payload = TemplateObject(
            name=template_name, language=language_payload, components=components_payload
        )
