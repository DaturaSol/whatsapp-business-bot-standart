...
class UnaSusChapterMenu(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        options = {
            "title": "Menu",
            "rows": [
                {"title": "Continuar", "id": UnaSusContinueChapter.__name__, "description": "Continuar de onde parou."},
                {"title": "Unidade 1", "id": UnaSusChapterOne.__name__, "description": "Estatuto da Igualdade Racial"},
                {"title": "Unidade 2", "id": UnaSusChapterTwo.__name__, "description": "PNSIPN"},
                {"title": "Unidade 3", "id": UnaSusChapterThree.__name__, "description": "Mas o que aconteceu com a PNSIPN?"},
                {"title": "Unidade 4", "id": UnaSusChapterFour.__name__, "description": "Marca, Diretrizes e Objetivos da PNSIPN"},
                {"title": "Unidade 5", "id": UnaSusChapterFive.__name__, "description": "O Quesito Raça/Cor..."},
            ],
        }
...
class UnaSusChapterOneContent(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        if "Documentário_Luis_Gama" in videos:
            next_function = UnaSusVidLuisGamaDoc(self.db_session, self.client_session, self.registery, self.value)
            await next_function._fn()
        if "Filme_Doutor_Gama" in videos:
            next_function = UnaSusVidLuisGamaFilm(self.db_session, self.client_session, self.registery, self.value)
            await next_function._fn()
...
class UnaSusVidLuisGamaDoc(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        message = CTAUrlButtonMessage(to=wa_id, display_text=display_text, body_text=body_text, url=url)
        message.add_header(header)
        await message.send(self.client_session)
...
class UnaSusChapterTwoContent(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        if response_json.get("lei_cao"):
            next_function = UnaSusVidLeiCao(self.db_session, self.client_session, self.registery, self.value)
            await next_function._fn()
        if response_json.get("video_politica"):
            next_function = UnaSusVidPNSIPN(self.db_session, self.client_session, self.registery, self.value)
            await next_function._fn()
        if response_json.get("documento_historico"):
            next_function = UnaSusDocNorteador(self.db_session, self.client_session, self.registery, self.value)
            await next_function._fn()
...
class UnaSusDocNorteador(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        message = DocumentMessage(to=wa_id, filename=file_name, link=url, caption=caption)
        await message.send(self.client_session)
...
class UnaSusChapterFiveContent(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        if response_json.get("guia"):
            next_function = UnaSusDocGuia(self.db_session, self.client_session, self.registery, self.value)
            await next_function._fn()
        if response_json.get("abrasco"):
            next_function = UnaSusAbrasco(self.db_session, self.client_session, self.registery, self.value)
            await next_function._fn()
        if response_json.get("Portaria"):
            next_function = UnaSusPortaria(self.db_session, self.client_session, self.registery, self.value)
            await next_function._fn()
...
class UnaSusPortaria(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        display_text = "Ler"
        url = "https://bvsms.saude.gov.br/bvs/saudelegis/gm/2017/prt0344_01_02_2017.html"
        message = CTAUrlButtonMessage(to=wa_id, display_text=display_text, body_text=body_text, url=url)
        message.add_header(header)
        await message.send(self.client_session)
...
class UnaSusAbrasco(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        header = "Vídeo"
        body_text = "Agora da ABRASCO sobre a PNSIPN e o preenchimento do 	quesito raça-cor. "
        display_text = "Assistir"
...
class UnaSusExerciseMenu(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        options = {
            "title": "Menu",
            "rows": [
                {"title": "Continuar", "id": UnaSusContinueExercise.__name__, "description": "Continuar de onde parou."},
                {"title": "Unidade 1", "id": UnaSusExerciseOne.__name__, "description": "Estatuto da Igualdade Racial"},
                {"title": "Unidade 2", "id": UnaSusExerciseTwo.__name__, "description": "PNSIPN"},
                {"title": "Unidade 3", "id": UnaSusExerciseThree.__name__, "description": "Mas o que aconteceu com a PNSIPN?"},
                {"title": "Unidade 4", "id": UnaSusExerciseFour.__name__, "description": "Marca, Diretrizes e Objetivos da PNSIPN"},
                {"title": "Unidade 5", "id": UnaSusExerciseFive.__name__, "description": "O Quesito Raça/Cor..."},
            ],
        }
...
class UnaSusExerciseOneHandle(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        q1_handler = UnaSusExerciseOneQ1Answer(self.db_session, self.client_session, self.registery, self.value)
        await q1_handler._fn()
        
        q2_handler = UnaSusExerciseOneQ2Answer(self.db_session, self.client_session, self.registery, self.value)
...
class UnaSusExerciseOneQ1Answer(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        match q1_answer:
            case "a":
                handler = UnaSusE1Q1A(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "b":
                handler = UnaSusE1Q1B(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
                user.exercise_grade_una["E1"][0] = 1 # type: ignore
                flag_modified(user, "exercise_grade_una")
                await self.db_session.commit()
            case "c":
                handler = UnaSusE1Q1C(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "d":
                handler = UnaSusE1Q1D(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
...
class UnaSusExerciseOneQ2Answer(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        match q2_answer:
            case "a":
                handler = UnaSusE1Q2A(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "b":
                handler = UnaSusE1Q2B(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "c":
                handler = UnaSusE1Q2C(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
                user.exercise_grade_una["E1"][1] = 1 # type: ignore
                flag_modified(user, "exercise_grade_una")
                await self.db_session.commit()
            case "d":
                handler = UnaSusE1Q2D(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
...
class UnaSusExerciseTwoHandle(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        q1_handler = UnaSusExerciseTwoQ1Answer(self.db_session, self.client_session, self.registery, self.value)
        await q1_handler._fn()
        
        q2_handler = UnaSusExerciseTwoQ2Answer(self.db_session, self.client_session, self.registery, self.value)
        await q2_handler._fn()
...
class UnaSusExerciseTwoQ1Answer(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        match q1_answer:
            case "a":
                handler = UnaSusE2Q1A(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "b":
                handler = UnaSusE2Q1B(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
                user.exercise_grade_una["E2"][0] = 1 # type: ignore
                flag_modified(user, "exercise_grade_una")
                await self.db_session.commit()
            case "c":
                handler = UnaSusE2Q1C(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "d":
                handler = UnaSusE2Q1D(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
...
class UnaSusExerciseTwoQ2Answer(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        match q2_answer:
            case "a":
                handler = UnaSusE2Q2A(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "b":
                handler = UnaSusE2Q2B(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "c":
                handler = UnaSusE2Q2C(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
                user.exercise_grade_una["E2"][1] = 1 # type: ignore
                flag_modified(user, "exercise_grade_una")
                await self.db_session.commit()
            case "d":
                handler = UnaSusE2Q2D(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
...
class UnaSusExerciseThreeHandle(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        q1_handler = UnaSusExerciseThreeQ1Answer(self.db_session, self.client_session, self.registery, self.value)
        await q1_handler._fn()
        
        q2_handler = UnaSusExerciseThreeQ2Answer(self.db_session, self.client_session, self.registery, self.value)
        await q2_handler._fn()
...
class UnaSusExerciseThreeQ1Answer(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        match q1_answer:
            case "a":
                handler = UnaSusE3Q1A(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "b":
                handler = UnaSusE3Q1B(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "c":
                handler = UnaSusE3Q1C(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
                user.exercise_grade_una["E3"][0] = 1 # type: ignore
                flag_modified(user, "exercise_grade_una")
                await self.db_session.commit()
            case "d":
                handler = UnaSusE3Q1D(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
...
class UnaSusExerciseThreeQ2Answer(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        match q2_answer:
            case "a":
                handler = UnaSusE3Q2A(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "b":
                handler = UnaSusE3Q2B(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "c":
                handler = UnaSusE3Q2C(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
                user.exercise_grade_una["E3"][1] = 1 # type: ignore
                flag_modified(user, "exercise_grade_una")
                await self.db_session.commit()
            case "d":
                handler = UnaSusE3Q2D(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
...
class UnaSusExerciseFourHandle(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        q1_handler = UnaSusExerciseFourQ1Answer(self.db_session, self.client_session, self.registery, self.value)
        await q1_handler._fn()
        
        q2_handler = UnaSusExerciseFourQ2Answer(self.db_session, self.client_session, self.registery, self.value)
        await q2_handler._fn()
...
class UnaSusExerciseFourQ1Answer(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        match q1_answer:
            case "a":
                handler = UnaSusE4Q1A(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "b":
                handler = UnaSusE4Q1B(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
                user.exercise_grade_una["E4"][0] = 1 # type: ignore
                flag_modified(user, "exercise_grade_una")
                await self.db_session.commit()
            case "c":
                handler = UnaSusE4Q1C(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "d":
                handler = UnaSusE4Q1D(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
...
class UnaSusExerciseFourQ2Answer(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        match q2_answer:
            case "a":
                handler = UnaSusE4Q2A(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "b":
                handler = UnaSusE4Q2B(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "c":
                handler = UnaSusE4Q2C(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
                user.exercise_grade_una["E4"][1] = 1 # type: ignore
                flag_modified(user, "exercise_grade_una")
                await self.db_session.commit()
            case "d":
                handler = UnaSusE4Q2D(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
...
class UnaSusExerciseFiveHandle(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        q1_handler = UnaSusExerciseFiveQ1Answer(self.db_session, self.client_session, self.registery, self.value)
        await q1_handler._fn()
        
        q2_handler = UnaSusExerciseFiveQ2Answer(self.db_session, self.client_session, self.registery, self.value)
        await q2_handler._fn()
...
class UnaSusExerciseFiveQ1Answer(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        match q1_answer:
            case "a":
                handler = UnaSusE5Q1A(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "b":
                handler = UnaSusE5Q1B(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
                user.exercise_grade_una["E5"][0] = 1 # type: ignore
                flag_modified(user, "exercise_grade_una")
                await self.db_session.commit()
            case "c":
                handler = UnaSusE5Q1C(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "d":
                handler = UnaSusE5Q1D(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
...
class UnaSusExerciseFiveQ2Answer(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        match q2_answer:
            case "a":
                handler = UnaSusE5Q2A(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "b":
                handler = UnaSusE5Q2B(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
            case "c":
                handler = UnaSusE5Q2C(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
                user.exercise_grade_una["E5"][1] = 1 # type: ignore
                flag_modified(user, "exercise_grade_una")
                await self.db_session.commit()
            case "d":
                handler = UnaSusE5Q2D(self.db_session, self.client_session, self.registery, self.value)
                await handler._fn()
...
class UnaCheckCongratulations(ScriptBaseModel):
    ...
    async def _fn(self):
        ...
        exercises_map = {
            "E1": ("Exercicio 1 Questão 1, faltante.", "Exercicio 1 Questão 2, faltante."),
            "E2": ("Exercicio 2 Questão 1, faltante.", "Exercicio 2 Questão 2, faltante."),
            "E3": ("Exercicio 3 Questão 1, faltante.", "Exercicio 3 Questão 2, faltante."),
            "E4": ("Exercicio 4 Questão 1, faltante.", "Exercicio 4 Questão 2, faltante."),
            "E5": ("Exercicio 5 Questão 1, faltante.", "Exercicio 5 Questão 2, faltante."),
        }
